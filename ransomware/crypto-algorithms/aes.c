...
* AES - CBC
*******************/
int aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORDA key[], int keysize, const BYTE iv[])
{
  ...
  //printf("b4 for loop in aes_enc_cbc\n");
...
int aes_encrypt_cbc_mac(const BYTE in[], size_t in_len, BYTE out[], const WORDA key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
int aes_decrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const WORDA key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORDA key[], int keysize, const BYTE iv[])
{
...
void aes_decrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const WORDA key[], int keysize, const BYTE iv[])
{
...
int aes_encrypt_ccm(const BYTE payload[], WORDA payload_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE out[], WORDA *out_len,
                    WORDA mac_len, const BYTE key_str[], int keysize)
{
...
int aes_decrypt_ccm(const BYTE ciphertext[], WORDA ciphertext_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE plaintext[], WORDA *plaintext_len,
                    WORDA mac_len, int *mac_auth, const BYTE key_str[], int keysize)
{
...
void aes_key_setup(const BYTE key[], WORDA w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	WORDA temp,Rcon[]={0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
	...
	for (idx = Nk; idx < Nb * (Nr+1); ++idx) {
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = SubWord(KE_ROTWORDA(temp)) ^ Rcon[(idx-1)/Nk];
...
void AddRoundKey(BYTE state[][4], const WORDA w[])
{
	BYTE subkey[4];
...
void aes_encrypt(const BYTE in[], BYTE out[], const WORDA key[], int keysize)
{
	BYTE state[4][4];
...
void aes_decrypt(const BYTE in[], BYTE out[], const WORDA key[], int keysize)
{
	BYTE state[4][4];
